...
import { Action, ToastProps } from "@radix-ui/react-toast"
// Inspired by react-hot-toast library
import * as React from "react"
import { useState, useEffect } from "react"

// import type { ToastAction, ToastProps } from "@/components/ui/toast"
const TOAST_LIMIT = 1
const TOAST_REMOVE_DELAY = 1000000

type ToastVariant = "default" | "destructive" | "success"

interface ToastPropsNew {
  title: string
  description: string
...
interface Toast extends ToastPropsNew {
  id: string
  visible: boolean
}
...
function useToast() {
  ...
  const [toastsNew, setToasts] = useState<Toast[]>([])
  ...
  const toastNew = ({ title, description, variant = "default", duration = 5000 }: ToastPropsNew) => {
