...
export default function FormActualizarServicio({ servicio_id, onSuccess, openDialog = false, setOpenDialog }: { servicio_id: number, onSuccess: (NewServicio: ServicioType) => void, openDialog: boolean, setOpenDialog: (open: boolean) => void }) {
    ...
    const [State_Form, SetState_Form] = useState<Omit<ServicioType, 'categorias_servicio'>>({})
    ...
    const FN_GET_SERVICIO = async () => {
        const res = await SERVICIOS_SERVICES.GET_SERVICIO_BY_ID(servicio_id);
        delete res.categorias_servicio
        SetState_Form(res)
        ...
        <Dialog open={openDialog} onOpenChange={setOpenDialog}>
            ...
            <DialogContent className="sm:max-w-[600px]">
                ...
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input value={State_Form?.nombre} id="nombre" placeholder="Nombre del servicio"
                                onChange={(e) => SetState_Form({ ...State_Form, nombre: e.target.value })} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="categoria">Categoría</Label>
                            <Select value={State_Form?.categoria_servicio_id?.toString()} onValueChange={(e) => SetState_Form({ ...State_Form, categoria_servicio_id: parseInt(e) })}>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <div className="space-y-2">
                            ...
                            <Input value={State_Form?.precio} id="precio" type="number" placeholder="0.00" onChange={(e) => SetState_Form({ ...State_Form, precio: parseFloat(e.target.value) })} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="tiempo" >Tiempo Estimado</Label>
                            <Input value={State_Form?.tiempo_estimado} id="tiempo" type="number" placeholder="Selecciona la cantidad de tiempo" onChange={(e) => SetState_Form({ ...State_Form, tiempo_estimado: parseFloat(e.target.value) })} />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="duracion">Duracion</Label>
                            <Select value={State_Form?.tipo_tiempo_estimado} onValueChange={(e) => SetState_Form({ ...State_Form, tipo_tiempo_estimado: e })}>
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea value={State_Form?.descripcion} id="descripcion" placeholder="Descripción detallada del servicio" onChange={(e) => SetState_Form({ ...State_Form, descripcion: e.target.value })} />
                    </div>
                    <div className="space-y-2">
                ...
                <DialogFooter>
                    ...
                    <Button disabled={State_Loadding} onClick={() => FN_ACTUALIZAR_SERVICIO()}>{State_Loadding ? 'Procesando...' : 'Actualizar Servicio'}</Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
    )
